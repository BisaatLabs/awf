...
export function ProductCard({ product }: { product: Product }) {
  const imageUrl = getProductImage(product.images[0], { width: 600 });

  return (
    <article className="group flex flex-col">
      <Link
        href={`/products/${product.slug}`}
        className="relative block aspect-[1.12] w-full overflow-hidden rounded-sm bg-[#f5f2eb] p-3 transition duration-500 hover:shadow-md"
      >
        <Image
          src={imageUrl}
          alt={product.name}
        ...
        <span className="absolute bottom-4 right-4 flex h-10 w-10 translate-y-2 items-center justify-center bg-[var(--ivory)] opacity-0 shadow-sm transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
      ...
      <div className="flex items-start justify-between gap-4 border-b border-[var(--line)] py-5">
        <div>
          <h3 className="font-display text-xl leading-snug text-[var(--charcoal)]">
            <Link
              href={`/products/${product.slug}`}
